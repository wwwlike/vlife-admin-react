...
export default ({
      ...
      effects() {
        onFormInit((form) => {}),
          onFormValuesChange((form) => {
            if (onDataChange) onDataChange({ ...form.values });
          });
      },
    });
  }, [modelInfo]);
  ...
  //  * 单个表单属性组合/覆盖
  //  */
  // const fieldInfos = useMemo((): FormFieldVo[] => {
  //   if(modelInfo&&modelInfo.fields.length>0){

  //   }
  //   return modelInfo?.fields.filter((f) => f.uiState !== "hide");
  // }, [modelInfo?.fields]);

  const schema = useMemo(() => {
    const pp: any = {};
    modelInfo?.fields
      .filter((f) => f.uiState !== "hide")
      .forEach((f) => {
        const prop: any = (pp[f.fieldName] = {});
        prop.title = f.title;
        prop["x-decorator"] = "FormItem";

        //传入的附加属性放入到组件props里的当前组件fieldName字段之上
        if (f.props) {
        ...
        if (f.component) {
          ...
          if (f.dictCode) prop.enum = fieldEnum(f.dictCode);
          else
            prop.enum = [
              { label: "是", value: true },
              { label: "否", value: false },
